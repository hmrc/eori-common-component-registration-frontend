...
class MappingsSpec extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks with OptionValues with Mappings {
  ...
  val form = Form("value" -> localDate(emptyKey = "doe.error.empty-date", invalidKey = "doe.error.invalid-date"))
  ...
  "must fail to bind a date with an invalid day" in {
    ...
    forAll(validData -> "valid date", invalidField -> "invalid field") { (date, field) =>
      ...
      result.errors must contain(FormError("value", "doe.error.invalid-date", List.empty))
    }
  }
  ...
  "must fail to bind an invalid day and month" in {
    ...
    forAll(validData -> "valid date", invalidField -> "invalid day", invalidField -> "invalid month") { (date, day, month) =>
      ...
      result.errors must contain only FormError("value", "doe.error.invalid-date", List.empty)
    }
  }
