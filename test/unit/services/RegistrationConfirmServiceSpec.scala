...
class RegistrationConfirmServiceSpec extends UnitSpec with MockitoSugar with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  "currentSubscriptionStatus" should {
    "get registration details from cache and call SubscriptionStatusService with SAP number" in {
      ...
      inOrder
        ...
        .getStatus(meq("taxPayerID"), meq(TaxPayerId(sapNumber).mdgTaxPayerId))(meq(hc), meq(mockRequest))
    }
    ...
    "propagate SubscriptionStatusService access error" in {
      mockGetStatus(mock[PreSubscriptionStatus])
      when(
        mockSubscriptionStatusService.getStatus(meq("taxPayerID"), meq(TaxPayerId(sapNumber).mdgTaxPayerId))(meq(hc), meq(mockRequest))
    ...
    def mockGetStatus(expectedResult: PreSubscriptionStatus): Unit = {
      ...
      when(mockCdsFrontendDataCache.registrationDetails(meq(mockRequest))).thenReturn(Future.successful(mockRegistrationDetails))
      when(
        mockSubscriptionStatusService.getStatus(meq("taxPayerID"), meq(TaxPayerId(sapNumber).mdgTaxPayerId))(meq(hc), meq(mockRequest))
  ...
  "clearRegistrationData" should {
    "call registration data clean service" in {
      ...
      verify(mockClearCacheAndRegistrationIdentificationService).clear()(meq(hc), meq(mockRequest))
    }

    "return success when data clean succeeds" in {
      when(mockClearCacheAndRegistrationIdentificationService.clear()(meq(hc), meq(mockRequest)))
