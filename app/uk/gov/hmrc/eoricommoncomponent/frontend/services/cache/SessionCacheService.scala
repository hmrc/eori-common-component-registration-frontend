...
import uk.gov.hmrc.eoricommoncomponent.frontend.controllers.routes.{ApplicationController, IndStCannotRegisterUsingThisServiceController, YouCannotChangeAddressController}
...
class SessionCacheService @Inject() (
  ...
  private def matchOnId(ninoOrUtr: NinoOrUtr, groupId: GroupId, postcode: String)(implicit
    ...
    retrieveNameDobFromCache().flatMap(ind =>
  ...
  private def matchIndDobAndPostCode(
    ...
    matchingResponse.registerWithIDResponse.responseDetail.exists(detail =>
