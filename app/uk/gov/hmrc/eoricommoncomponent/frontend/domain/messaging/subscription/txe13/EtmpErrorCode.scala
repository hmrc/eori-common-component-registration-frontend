...
object EtmpErrorCode {
  ...
  /** We don't know what the key will be at the top level hence we retrieve the key first then do the look up by the key since we are not interested in the key
    * itself
    *
    * key examples according to API Documentation: errorcode007, errorcode133, errorcode135, invalidEdgeCaseType, invalidIoMPostcode
    */
