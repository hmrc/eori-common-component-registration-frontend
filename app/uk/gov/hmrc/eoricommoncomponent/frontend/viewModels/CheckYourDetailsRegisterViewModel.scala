...
class CheckYourDetailsRegisterConstructor @Inject()(
  ...
  def generateViewModel(cdsOrgType: Option[CdsOrganisationType],
    ...
    val isIomOrganisation = cdsOrgType.contains(CdsOrganisationType.IsleOfManOrganisation)
    val isIomSoleTrader = cdsOrgType.contains(CdsOrganisationType.IsleOfManSoleTrader)
    val isIomIndividual = cdsOrgType.contains(CdsOrganisationType.IsleOfManIndividual)
