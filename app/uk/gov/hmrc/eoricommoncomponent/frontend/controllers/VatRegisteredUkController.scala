...
import uk.gov.hmrc.eoricommoncomponent.frontend.controllers.routes.{ApplicationController, ContactDetailsController, VatDetailsController, YourVatDetailsController}
    ...
    authAction.enrolledUserWithSessionAction(service) { implicit request => (_: LoggedInUserWithEnrolments) =>
      vatRegisteredUkYesNoAnswerForm(requestSessionData.isPartnership)
        ...
        .fold(
          ...
          yesNoAnswer =>
            subscriptionDetailsService.cacheVatRegisteredUk(yesNoAnswer).flatMap { _ =>
              val result = (isInReviewMode, yesNoAnswer.isYes) match {
                ...
                case (true, false) =>
                  subscriptionDetailsService.clearCachedUkVatDetails.map(_ => ContactDetailsController.reviewForm(service).url)
                case (false, false) =>
                  subscriptionDetailsService.clearCachedUkVatDetails.map(_ => ContactDetailsController.createForm(service).url)
